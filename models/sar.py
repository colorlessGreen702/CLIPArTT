...
class SAR(nn.Module):
    ...
    def forward(self, x, text_x):
        ...
        for _ in range(self.steps):
            outputs, ema, reset_flag = forward_and_adapt_sar(x, text_x, self.model, self.optimizer, self.margin_e0, self.reset_constant_em, self.ema)
            if reset_flag:
                self.reset()
...
def forward_and_adapt_sar(x, text_x, model, optimizer, margin, reset_constant, ema):
    ...
    entropys = softmax_entropy(outputs.T)
    ...
    outputs2, _, _ = model(x, text_x)
    entropys2 = softmax_entropy(outputs2.T)
